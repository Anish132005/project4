...
public class SupplierModel {
	...
	public int nextPK() throws DatabaseException {
		...
		String sql = "SELECT MAX(ID) FROM st_supplier";
	...
	public long add(SupplierBean bean) throws ApplicationException, DuplicateRecordException {
		...
		String sql = "INSERT INTO st_supplier VALUES(?,?,?,?,?)";
		...
		try {
			...
			pstmt.setDate(4, new java.sql.Date(bean.getRegistrationdate().getTime()));
			pstmt.setInt(5, bean.getPayment());
	...
	public void delete(SupplierBean bean) throws ApplicationException {
		...
		String sql = "DELETE FROM st_supplier WHERE ID=?";
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
	...
	public void update(SupplierBean bean) throws ApplicationException, DuplicateRecordException {
		...
		String sql = "UPDATE st_supplier SET Name=?,Category=?,Registrationdate=?,payment=? WHERE ID=?";
		Connection conn = null;
		...
		try {
			...
			pstmt.setDate(3, new java.sql.Date(bean.getRegistrationdate().getTime()));
			pstmt.setInt(4, bean.getPayment());
	...
	public List search(SupplierBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getRegistrationdate() != null && bean.getRegistrationdate().getTime() > 0) {
				Date d = new Date(bean.getRegistrationdate().getDate());
				sql.append(" AND Registrationdate like '" + new java.sql.Date(bean.getRegistrationdate().getTime())
						+ "%'");
			}

			if (bean.getPayment() != null && bean.getPayment() > 0) {
				sql.append(" AND payment = " + bean.getPayment());
			}
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setRegistrationdate(rs.getDate(4));
				bean.setPayment(rs.getInt(5));
