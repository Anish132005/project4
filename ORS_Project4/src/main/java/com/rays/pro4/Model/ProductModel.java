...
public class ProductModel {
	...
	public long add(ProductBean bean) throws Exception {
		...
		pstmt.setDate(4, new java.sql.Date(bean.getProductDate().getTime()));
	...
	public void update(ProductBean bean) throws Exception {
		...
		PreparedStatement pstmt = conn.prepareStatement(
				"update st_product set productName = ?, productPrice = ?, productDate = ?, productCategory = ? where id = ?");

		pstmt.setString(1, bean.getProductName());
		pstmt.setString(2, bean.getProductPrice());
		pstmt.setDate(3, new java.sql.Date(bean.getProductDate().getTime()));
	...
	public ProductBean findByPK(long pk) throws Exception {
		...
		while (rs.next()) {
			...
			bean.setProductDate(rs.getDate(4));
	...
	public List search(ProductBean bean, int pageNo, int pageSize) throws Exception {
		...
		if (bean != null) {
			...
			if (bean.getProductDate() != null && bean.getProductDate().getTime() > 0) {
				Date d = new Date(bean.getProductDate().getTime());
				sql.append(" AND productDate = '" + d + "'");
