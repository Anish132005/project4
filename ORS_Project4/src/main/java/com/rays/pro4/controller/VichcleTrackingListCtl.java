...
@WebServlet(name = "VichcleTrackingListCtl", urlPatterns = { "/ctl/VichcleTrackingListCtl" })
public class VichcleTrackingListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		...
		/* bean.setId(DataUtility.getLong(request.getParameter("prlist"))); */

		bean.setVichcleId(DataUtility.getInt(request.getParameter("VichcleId")));

		System.out.println("anish...edrfgyjkl.rdtyui" + bean.getVichcleId());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SEARCH.equalsIgnoreCase(op)) {
			...
			ServletUtility.redirect(ORSView.VICHCLE_TRACKING_CTL, request, response);
			return;
			...
			if (ids != null && ids.length > 0) {
				VichcleTrackingBean deletebean = new VichcleTrackingBean();
				for (String id : ids) {
					...
					ServletUtility.setSuccessMessage("Vichcle is Deleted Successfully", request);
				}
			} else {
	...
	protected String getView() {
		...
		return ORSView.VICHCLE_TRACKING_LIST_VIEW;
	}
